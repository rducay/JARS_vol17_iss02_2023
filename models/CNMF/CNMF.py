# Functions to implement CNMF
#
# Author:  Naoto Yokoya  2011-2016

import numpy as np
from scipy.special import erfinv

def CNMF_fusion(HSI,MSI,mask=0,verbose='off',MEMs=0):
    '''
    ...
    for b in range(bands1):
        msi = MSI[:,:,b].reshape(rows1,cols1,1).copy()
        msi = msi - R[b,-1]
        msi[np.nonzero(msi<0)] = 0
        MSI[:,:,b] = np.squeeze(msi.copy(), axis=2) #use when sharpening using HRPAN
        #MSI[:,:,b] = msi.copy() #use when sharpening using HRMSI
    R = R[:,0:bands2]
