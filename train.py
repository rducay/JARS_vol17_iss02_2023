...
def train(train_list, 
    ...
    #train_image_size = 256 #KEEP THIS FIXED
    train_image_size = 128 #KEEP THIS FIXED
    h, w = train_ref.size(2), train_ref.size(3)
    h_str = random.randint(0, h-train_image_size-1)
    w_str = random.randint(0, w-train_image_size-1)

    train_lr  = train_ref[:, :, h_str:h_str+train_image_size, w_str:w_str+train_image_size]
    train_ref = train_ref[:, :, h_str:h_str+train_image_size, w_str:w_str+train_image_size]
    train_lr  = F.interpolate(train_ref, scale_factor=1/(scale_ratio*1.0))
    train_hr  = train_hr[:, :, h_str:h_str+train_image_size, w_str:w_str+train_image_size]
